...
# Update everything (brew, mac app store, mise, rust, completions)
update: update-brew update-mas update-mise update-rust update-completions
...
# Regenerate vendored zsh completions
update-completions:
    #!/usr/bin/env bash
    set -euo pipefail
    completions_dir="{{dotfiles_dir}}/zsh/functions"
    gen() {
        local name="$1" cmd="$2"
        if command -v "$name" >/dev/null 2>&1; then
            echo "Updating $name completion..."
            { echo "#compdef $name"; echo '# shellcheck disable=all'; echo "# Generated by: $cmd"; eval "$cmd" | grep -v '^#compdef' | sed '1{/^$/d;}'; } > "$completions_dir/_$name"
        else
            echo "$name not found, skipping"
        fi
    }
    gen roborev "roborev completion zsh"
    gen rustup "rustup completions zsh"
    gen sops   "sops completion zsh"
