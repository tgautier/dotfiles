...
| `mode` | Required rendered mode: `file`, `executable`, or `symlink` |
...
Every `file` and `executable` shadow source under `home/` is a relative symlink to its canonical top-level `source` path, so each dotfile has exactly one tracked copy. Chezmoi reads through the link and renders a regular file with the canonical content. The one `symlink`-mode row is the mise template, which stays a real file because its content is a template that renders a symlink target, not a copy of a source.
...
1. Validates every manifest row against the repository source tree, requiring each `file` and `executable` shadow source to be a symlink that resolves to its manifest `source` and the template row to stay a real file
...
1. Runs companion available, conflicting-package, absent, non-directory, stale, ownership-rejection, source-rejection, and output-withholding fixtures plus sabotage fixtures for a misnamed target, omitted shadow rows, a duplicate row, a missing source, an extra source, a wrong template symlink destination, a reintroduced shadow copy, a mispointed shadow symlink, and a symlinked template source
