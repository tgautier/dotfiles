...
Before the first real apply, create and verify the exact rcm link backup described in [Chezmoi cutover backup and rcm restore](chezmoi-cutover-backup.md). Then follow [Chezmoi operator cutover](chezmoi-operator-cutover.md) to bind the reviewed output to that backup, apply both sources twice, preserve the working state, and recover after failure or interruption.
