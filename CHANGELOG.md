...
- Approval-bound public/private chezmoi application with automatic rcm recovery. The operator first reviews exact status, diff, and dry-run output after backup verification and the isolated canary, whose tool path selects the exact chezmoi and `lsrc` executables bound by the approval. The resulting digest binds HOME, both source trees and ownership contracts, the backup, canonical operator helper, canary, recovery code and rcm configuration, selected executable paths and bytes, and rendered output. Apply reruns the canary, recomputes that evidence without exposing inspection output, verifies the backup again, and revalidates an approval-derived execution contract immediately before every later chezmoi invocation. It applies public then private twice and requires empty status, diff, and dry-run state after each pass. A standard nonblocking file lock serializes shared state. Every subprocess runs in a dedicated process group with bounded group-wide cleanup before recovery or the next phase, and captured output crossing its limit terminates that group immediately. Any mutation or post-apply failure invokes the tested complete rcm restore. Success is committed while handled signals are blocked, and the one-shot operator retains those handlers until process exit so teardown cannot turn a completed cutover into an ambiguous failure. Fixtures cover strict recovery-summary parsing, live output bounds, exact canary tool selection, approval, operator, executable-path, and post-approval source drift, public-only and two-source order, successful idempotence, timeout, descendant cleanup before recovery, deferred termination before and during restore, committed completion handoff and handler retention, concurrent invocation, partial apply, post-apply drift, successful recovery, and failed recovery. The tracked config and helper remain repository-only through explicit rcm exclusions. Rcm, `just link`, and `just setup` remain unchanged until live cutover evidence is reviewed ([#232](https://github.com/tgautier/dotfiles/issues/232)).
- Optional two-source orchestration in the public chezmoi canary. A stdlib Python bridge runs the companion ownership preflight before the first public apply and the companion's isolated source canary after the public second apply is a no-op. It binds both phases to one opaque identity covering the checkout root, Git state, tracked and nonignored source content, every importable top-level Python module or package tree even when ignored, and in-checkout symlink referents; rejects symlinked checker import paths; replaces inherited HOME and XDG paths; scrubs Python, virtual-environment, and Git state; withholds all companion output; and performs bounded process-group cleanup after every checker exit. An absent companion reports a generic skip and preserves the complete public-only canary only while it remains absent for both phases. That checkpoint left rcm as the live deployment owner and did not apply chezmoi to the real HOME ([#232](https://github.com/tgautier/dotfiles/issues/232)).
