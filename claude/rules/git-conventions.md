...
- **Every push requires a review cycle** — run `roborev review --branch --agent <agent>` for each configured agent. Review findings, fix or defer per user decision. This is not optional and not just for the first push — every push triggers a review. See `/roborev` for the full workflow and agent list
