...
- Approval-bound public/private chezmoi application with automatic rcm recovery. The operator first reviews exact status, diff, and dry-run output after the isolated canary and backup verification. The resulting digest binds HOME, both source trees and ownership contracts, the backup, canary, recovery code and rcm configuration, selected executables, and rendered output. Apply reruns the canary, recomputes that evidence without exposing inspection output, verifies the backup again, applies public then private twice, and requires empty status, diff, and dry-run state after each pass. A standard nonblocking file lock serializes shared state, every subprocess invocation is time-bounded, and any mutation or post-apply failure invokes the tested complete rcm restore. Fixtures cover strict recovery-summary parsing and output bounds, approval drift, public-only and two-source order, successful idempotence, timeout, deferred termination signals, concurrent invocation, partial apply, post-apply drift, successful recovery, and failed recovery. The tracked config and helper remain repository-only through explicit rcm exclusions. Rcm, `just link`, and `just setup` remain unchanged until live cutover evidence is reviewed ([#232](https://github.com/tgautier/dotfiles/issues/232)).
